...
public class Movement_FPS2 : MonoBehaviour
{
    //Static variables 
    ...
    ****************/
     ...
     */
    private MasterInput2 inputController2;
    ...
    private void Awake()
    {
        ...
        inputController2 = new MasterInput2();
    }
    ...
    private void OnEnable()
    {
        ...
        //vectors
        //Move
        inputController2.Player.Move.performed += ctx /*grabs context for input*/ => moveInputForce = ctx.ReadValue<Vector2>();
        inputController2.Player.Move.canceled += _ /*context is not used, so we put a _ to signify. */ => moveInputForce = Vector2.zero;

        //Look
        inputController2.Player.Look.performed += ctx /*grabs context for input*/ => MoveCamera(ctx.ReadValue<Vector2>());
        
        //bools
        
        //Jump
        inputController2.Player.Jump.performed += _ /*context is not used, so we put a _ signify. */ => Jump();

        //Switch Perspective
        inputController2.Player.SwitchPerspective.performed += _ /*context is not used, so we put a _ to signify. */ => SwitchCameraPos();

        inputController2.Player.Use.performed += _ => Use();

        inputController2.Player.LockCursor.performed += _ => ToggleCursorLock();

        inputController2.Player.Sprint.performed += _ => switchSprint();
    }
    ...
    private void OnDisable()
    {
        ...
        //vectors
        //Move
        inputController2.Player.Move.performed -= ctx /*grabs context for input*/ => movePlayer(ctx.ReadValue<Vector2>());
        inputController2.Player.Move.canceled -= _ /*context is not used, so we put a _ to signifiy.. */ => moveInputForce = Vector2.zero;
        ...
        //bools
        //Jump
        inputController2.Player.Jump.performed -= _ /*context is not used, so we put a _ to signifiy.. */ => Jump();
        //Swtch Perspective
        inputController2.Player.SwitchPerspective.performed -= _ /*context is not used, so we put a _ to signifiy.. */  => SwitchCameraPos();
